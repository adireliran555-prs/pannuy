...
import { generateOtp, hashOtp, isAdminPhone } from "@/lib/auth";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Pre-flight: deny non-admin phones up front so we don't leak SMS to
    // anyone who guesses the route.
    if (!isAdminPhone(phone)) {
      // Same shape as success — don't reveal which numbers are admin.
    ...
    console.error("[POST /api/admin/auth/send-otp]", err);
